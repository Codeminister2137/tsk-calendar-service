...
class TestOrderByAttribute:
    ...
    def test_should_raise_value_error_on_unsupported_mode_without_existing_attribute(self):
...
class TestGroupByAttribute:
    ...
    def test_should_return_dictionary_grouped_by_status(self):
        mode = "status"
        actual_value = self.calendar.group_by_attribute(tasks=self.calendar.tasks, mode=mode)
        expected_value = defaultdict(list)
...
class TestFilterByAttribute:
    ...
    def test_should_return_list_filtered_by_name(self):
        mode = "name"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=self.target_task.name)
    ...
    def test_should_return_list_filtered_by_expected_duration(self):
        mode = "expected_duration"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=self.target_task.expected_duration)
        for task in actual_value:
    ...
    def test_should_return_list_filtered_by_actual_duration(self):
        mode = "actual_duration"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=self.target_task.actual_duration)
        for task in actual_value:
            assert task.actual_duration == self.target_task.actual_duration

    def test_should_return_list_filtered_by_category(self):
        mode = "category"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=self.target_task.categories[0])
        for task in actual_value:
            assert self.target_task.categories[0] in task.categories

    def test_should_return_list_filtered_by_deadline_year(self):
        mode = "deadline_year"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=self.target_task.deadline)
        for task in actual_value:
    ...
    def test_should_return_list_filtered_by_deadline_month(self):
        ...
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=self.target_task.deadline)
        for task in actual_value:
            assert task.deadline.year == self.target_task.deadline.year and task.deadline.month == self.target_task.deadline.month

    def test_should_return_list_filtered_by_deadline_week(self):
        mode = "deadline_week"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=self.target_task.deadline)
        for task in actual_value:
            assert task.deadline.isocalendar()[:2] == self.target_task.deadline.isocalendar()[:2]
    ...
    def test_should_return_list_filtered_by_priority(self):
        mode = "priority"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=self.target_task.priority)
        for task in actual_value:
            assert task.priority == self.target_task.priority

    def test_should_return_list_filtered_by_status(self):
        mode = "status"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=self.target_task.status.name)
        for task in actual_value:
            assert task.status.name == self.target_task.status.name

    def test_should_return_list_filtered_by_notifications_quantity(self):
        mode = "notifications_quantity"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=len(self.target_task.notifications))
        for task in actual_value:
    ...
    def test_should_return_list_filtered_by_categories_quantity(self):
        mode = "categories_quantity"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=len(self.target_task.categories))
        for task in actual_value:
    ...
    def test_should_raise_value_error_on_unsupported_mode_with_existing_attribute(self):
        mode = "notifications"
        with pytest.raises(ValueError):
            self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=len(self.target_task.notifications))
        mode = "categories"
        with pytest.raises(ValueError):
            self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target=len(self.target_task.notifications))
    ...
    def test_should_return_empty_list_when_filtering_by_non_matching_target(self):
        mode = "name"
        actual_value = self.calendar.filter_by_attribute(tasks=self.tasks, mode=mode, target="Absolutely nonexistant target")
